...
\begin{itemize}
    ...
    \begin{itemize}
        \item Major choice and labor market outcomes \citep{arcidiacono2016college,wiswall2015determinants}
        \item Persistence and dropout decisions \citep{stinebrickner2012learning}
...
\textbf{Setting:} Primary/secondary schools with digital gradebooks but low parent--school communication. Randomization stratified by school characteristics.
...
\begin{itemize}
    \item T1 vs.\ C: isolates \emph{channel} (attention)
    \item T2 vs.\ C: isolates \emph{content} (beliefs)
    \item T3 vs.\ C: joint effect (replication)
    \item T4 vs.\ T3: frequency / sustained attention
\end{itemize}
...
\[
Y_i = \alpha + \beta_1 \cdot \mathbf{1}[\text{SMS}]_i + \beta_2 \cdot \mathbf{1}[\text{Detailed}]_i + \beta_3 \cdot \mathbf{1}[\text{SMS} \times \text{Detailed}]_i + X_i'\gamma + \epsilon_i
\]
...
\begin{itemize}
    ...
    \item T4 arm: adds frequency dimension
\end{itemize}

\bigskip

\textbf{Additional data:} Elicit baseline beliefs $\hat{a}_i^{\text{pre}}$ to observe belief gap $\Rightarrow$ helps separate biased priors from distorted updating
...
\textbf{Assumptions:}
\begin{itemize}
    \item MDE = 0.10 SD (individual mechanisms are smaller than joint)
    \item Significance level $\alpha = 0.05$, power = 0.80
    \item ICC $\rho = 0.05$ (school-level clustering)
    \item 20 students per school cluster
\end{itemize}
...
\]
where $\text{DEFF} = 1 + (20-1) \times 0.05 = 1.95$
...
\begin{itemize}
    ...
    \item Stratification by school SES helps reduce variance
\end{itemize}
...
\textbf{Summary:}
\begin{itemize}
    \item Existing evidence shows information treatments improve educational outcomes
    \item But we cannot distinguish \emph{why}: attention, beliefs, or their interaction
...
\textbf{Contribution:}
\begin{itemize}
    \item Moves beyond reduced-form to structural understanding of why information works
    \item Low-cost interventions with potentially large returns
    \item Framework generalizable to other parental/educational information settings
\end{itemize}
