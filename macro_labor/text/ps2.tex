...
The remaining moments are also close to those reported in \citet{shimer2005}. The main small differences appear in the autocorrelations of $\frac{v}{u}$ and $f$, which may arise from the monthly calibration, the discretization of the productivity process, and simulation error. Overall, the replication matches the qualitative results in the original paper.

Despite matching the qualitative correlations, the model still generates too little volatility in labor market variables relative to the data, which is the essence of the Shimer puzzle.
...
keeping wages constant after a match is realized -following the idea of \citet{hall2005}. \\

\subsection*{c)}
Table (\ref{tab:proc_leisure}) follows the \citet{chodorowreich2016}
...
\subsection*{d)}
Table~\ref{tab:proc_beta} reports the results when the discount factor is allowed to vary with productivity according to
\[
\beta(z) = \bar{\beta} + \phi (z - \bar{z}),
\]
where $\bar{\beta}$ is the baseline monthly discount factor, $\bar{z}=1$ denotes mean productivity, and $\phi$ determines the sensitivity of discounting to the state of the economy. In periods of high productivity, agents discount future payoffs slightly less, which increases the continuation value of a match and may strengthen firms’ incentives to post vacancies.

However, under our calibration the variation in $\beta(z)$ is quantitatively small, so the effect on equilibrium outcomes is limited. As shown in Table~\ref{tab:proc_beta}, the volatility of unemployment, vacancies, and labor market tightness remains very close to the baseline case. Consequently, allowing the discount factor to be procyclical does not substantially amplify productivity shocks, and the model remains close to the baseline specification.
...
Comparing Tables~\ref{tab:baseline} and~\ref{tab:proc_leisure}, the results are very similar to the baseline, and in some dimensions volatility is even lower. This is consistent with the critique of \citet{chodorowreich2016}. If leisure is procyclical, so that $b(z)=\zeta z$, then the worker's outside option increases in good times. As a consequence, the match surplus $z-b(z)$ changes little over the cycle. Firms' profits therefore fluctuate less, vacancy creation reacts weakly, and the model continues to generate low labor-market volatility.

Finally, comparing Tables~\ref{tab:baseline} and~\ref{tab:proc_beta}, the results remain very close to those of the baseline model. Introducing a procyclical discount factor implies that agents value future match surplus slightly more in periods of high productivity. In principle, this could increase the incentive to create vacancies, since firms place a higher value on future profits. However, in our calibration the variation in the discount factor is small, so this mechanism has only a limited effect on equilibrium outcomes. As a result, the volatility of unemployment, vacancies, and labor market tightness remains very similar to the baseline case, and the model still generates only modest amplification of productivity shocks.
...
\input{hm.tex}
\input{hall.tex}
\input{proc_leisure.tex}
\input{proc_beta.tex}
%\input{proc_gamma.tex} %added as we did it before..., 
