...
\section*{Exercise 2}
In table~\ref{tab:transition} I show the results for $\rho$ and $\sigma^{\rho}$. We can see that the mean increase in miles of buses that don't invest is $0.19$ which is $1.900$ miles per bus. The $\sigma^{\rho}$ is  $0.11$, which is also reasonable. 

\section*{Exercise 3}
In table~\ref{tab:q3} I report the results of \(x_{iY}^{j}\) for \(j \in \{1,2,3\}\) and \(Y \in \{0,1\} \). We can see that the values are also intuitive. The replacement stream costs are higher for buses that replace today, as they assume the fixed cost of replacement. However, we can see that then their mileage stream has a lesser cost if they invest today, reflecting better health of the engine after replacement. Finally, The shock component differs slightly because the probability of future replacement differs across the two initial conditions, which affects the log probability term entering the expected value

\section*{Exercise 4}
Finally, in table~\ref{tab:q4} I show the results of the structural model. We can see that the replacement fixed cost is $9.55$ while the mileage cost $0.007$. Both are precisely estimated. The magnitudes \dots
