...
of the later market imposes no welfare cost. The framework 
...
\paragraph{Adding the government (positive side).} Suppose the government posts a price $p>I$.
...
money\footnote{The government and market prices are equated in equilibrium, so the pooled low types it buys ($\theta<p-S$) are paid a price above their fundamental value $\theta R_0$---hence it loses money on them.}; private buyers break even. This is the ``creates its own competition''
mechanism in its simplest form. Since the posted price pins down the financing cutoff one-for-one ($\theta^\ast=(p+S)/R_0$), choosing the price is the same as choosing $\theta^\ast$, which is how the next step frames the government's problem.

\paragraph{When does the government intervene? (normative side)} With cutoff $\theta^\ast$ and
...
\subsection{The general model: what the richer contract space changes}

The buyback case has a single instrument: a price. The government either buys
the whole asset or it does not, and the only sorting that happens is through who
chooses to sell. The general model adds a genuinely new instrument. Because part
of the legacy return is now contractible, the government (or the market) can
finance the firm while letting it retain a stake $y$ in the legacy
asset's success---``skin in the game.'' A financed type $\theta$ then either
shirks and gets $B$, or works and gets $b+\theta y$, so its rent is
\[
V(\theta y) \;=\; \max\{B,\; b+\theta y\}.
\]
The key point is that this stake is itself a screening device: since
$b+\theta y$ increases in $\theta$, a good type values keeping a stake more than
a bad type does. The government can therefore separate types by how much skin it
asks them to keep, rather than only by price.

Formally one now works with incentive-compatible contracts and the rents they
generate. Let $\Ug(\theta)$ be type $\theta$'s rent in the government mechanism
and $\Um(\theta;\Theta_m)$ her rent in the market when the holdout set is
$\Theta_m$. An equilibrium partitions types into those who join the government,
$\Theta_g$, and those who go to the market, $\Theta_m=\Theta\setminus\Theta_g$,
with $\Ug(\theta)\ge\Um(\theta;\Theta_m)\Rightarrow\theta\in\Theta_g$ and the
reverse for $\Theta_m$. The government picks an IC mechanism (a rent profile
$\Ug$) to maximize $W$ subject to this being the equilibrium of the fixed-point
problem---exactly the endogenous participation constraint of the introduction.

The screening stake reshapes the form of the optimum (figures~3-4 and Proposition~7). The worst
types are still bought out outright---a flat rent $U(\theta)=B$, no skin in the
game---but better types are now financed while keeping a graduated stake, so
their rent rises along $b+\theta y$. This is the \textbf{pecking order} (buy the
worst, recapitalize the middle, leave the best), and now it is not assumed but
derived: it is what optimal screening through revenue-sharing looks like.
For the same reason the optimal intervention is more extensive than under
buybacks: separating types by a retained stake is cheaper than separating them
by outright purchase, so the government optimally reaches more types.

The remaining lessons of the buyback
case survive the richer contract space essentially intact:
\begin{enumerate}[label=(\roman*),leftmargin=2.2em]
  ...
  the government reduces adverse selection enough to let it restart, but not so
  much as to make the intervention needlessly expensive.
  ...
  government still loses money on every financed type.
  \item \textbf{No desire to shut the market down.} If the law forbids
  expropriation (firms get at least $U_0(\theta)=\theta R_0$), the
  voluntary-participation constraint can be made costless: the government would
  gain nothing from the power to ban the market, though the market deeply shapes
  how it intervenes.
  ...
  intervention as gains are there much higher.
  \item \textbf{Intervention creates moral hazard.} The prospect of a bailout
  decreases ex-ante incentives to originate high-quality assets.
\end{enumerate}

A by-product (Section III) connects to Rothschild--Stiglitz: the
constrained-efficient allocation---highest surplus subject to seller IC and
buyer break-even---is itself an equilibrium of the laissez-faire market, and the
unique one surviving a ``robust choice'' refinement. The market left alone
already implements the best incentive-feasible allocation; the government's role
is only to relax the adverse-selection constraint, at a fiscal cost.
...
\begin{itemize}[leftmargin=1.4em]
  ...
  constrained optimum (figure 4 in the paper) tilts upward and is no longer constant but the logic is mostly unchanged.
  ...
  correlation it collapses to one dimension and the analysis goes through\footnote{A doubt however is what happens if $S(\theta)$ is exponentialy increasing in its argument\dots}.
...
\paragraph{Revelation principle and the uninformed-principal logic (Myerson 1982).}
...
the single most important ``twist'' on the basics of the course that this paper brings.
...
``skin in the game'' logic of the moral-hazard part of the course. Insight (vi)---intervention distorting ex-ante origination---is itself a moral-hazard result put on top of the screening problem. The main results of the paper,  however, focus on the adverse selection problem, which is far more important in the application.
%==================================================================
...
\begin{enumerate}[leftmargin=1.6em]
  ...
  selection, here is the optimal response. That said, the real contribution seems fundamentally
...
\item \textbf{Commitment.} The analysis assumes the government commits at stage~2
  to a mechanism it will not later revise. This is quite load-bearing: the clean timing
  that makes reservation utilities respond to the mechanism (rather than the
  reverse) requires the government to move first and stick to it. Tirole's own
  moral-hazard extension shows the bite---with commitment the government would
  choose a smaller, cheaper intervention, but that policy is time
  inconsistent, since ex post it wants to raise the price back to the level implied
  by~(2); anticipating the revision, firms behave as if there were no commitment,
  and the commitment gain evaporates. Real bailouts, renegotiated under political
  pressure (the drawn-out Japanese recapitalizations), look much more like the
  no-commitment case. Relaxing commitment fully, however, seems for future work.

\item \textbf{Specification of the outside option.} Several headline results rest on the fleeting-opportunity payoff $U_{0}=\theta R_{0}$ together with no-expropriation. Tirole claims (fn. 12) the analysis extends to the less-urgent case $U_{0}=\theta(R_{0}+\delta S)$; I'd have liked to see whether the 'no gain from shutting the market' conclusion is equally clean there, since a higher outside option changes what the government must concede.
